...
document.querySelector('.copy-cart-link')?.addEventListener('click', async (event) => {
  ...
  if (shouldUseWebShare()) {
    ...
    try {
      ...
      link.textContent = window.themeTranslations?.cart?.cartLinkCopySuccess || 'Link copied!';
      setTimeout(() => {
...
updateCartCounts = (cart) => {
  ...
  const cartCountElements = document.querySelectorAll('.cart-link span');
  const cartTotalElements = document.querySelectorAll('.header-subtotal-amount');
